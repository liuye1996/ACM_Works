...
int main()
{
	int n, m, sum, maxsum, allsum;
	string name, maxname, iswest, isleader;
	int avg, clscore, ispaper;
	cin >> n;
	while (n--)
	{
		maxsum = 0, allsum = 0, maxname = "";
		cin >> m;
		while (m--)
		{
			sum = 0;
			cin >> name >> avg >> clscore >> isleader >> iswest >> ispaper;
			if (avg > 80 && ispaper >= 1) sum += 8000;
			if (avg > 85 && clscore > 80) sum += 4000;
			if (avg > 90) sum += 2000;
			if (avg > 85 && iswest == "Y") sum += 1000;
			if (clscore > 80 && isleader == "Y") sum += 850;
			if (sum > maxsum) { maxsum = sum; maxname = name; }
			allsum += sum;
		}
		cout << maxname << endl << maxsum << endl << allsum <<endl;
	}
	//return 0;
	system("pause");
